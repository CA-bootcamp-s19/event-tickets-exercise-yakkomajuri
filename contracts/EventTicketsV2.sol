...
contract EventTicketsV2 {
    ...
    */
    ...
    function addEvent(string memory _description, string memory _websiteUrl, uint _nTicketsForSale) public onlyOwner returns (uint) {
        ...
        idGenerator++;
        emit LogEventAdded(_description, _websiteUrl, _nTicketsForSale, (idGenerator - 1));
    ...
    */
    function buyTickets(uint _eventId, uint _numberOfTickets) public payable {
        ...
        selectedEvent.sales += _numberOfTickets;
    ...
    */
    function getRefund(uint _eventId) public {
        Event storage selectedEvent = events[_eventId];
        uint ticketsToRefund = selectedEvent.buyers[msg.sender];
        require(ticketsToRefund > 0);
        uint valueToRefund = PRICE_TICKET * ticketsToRefund;
        msg.sender.transfer(valueToRefund);
        selectedEvent.sales -= ticketsToRefund;
        selectedEvent.buyers[msg.sender] = 0;
        emit LogGetRefund(msg.sender, _eventId, ticketsToRefund);
    }
